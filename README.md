...
Authenticated dashboard render timings can be benchmarked with:

```bash
PLAYWRIGHT_BASE_URL="https://your-preview-url.vercel.app" npm run test:perf
```
...
### Vercel + Neon performance notes

For Neon deployments, set `DATABASE_URL` to the pooled runtime connection string, usually the host containing `-pooler`. Set `DIRECT_DATABASE_URL` to the direct Neon connection string for Prisma migrations and direct maintenance operations. Do not hardcode either URL in source.

On Vercel Hobby/free plus Neon free or scale-to-zero, the first authenticated request after idle time can still be slow because both the serverless function and database may cold start. Authenticated App Router pages in this app read request headers and DB-backed auth state, so they are dynamic server renders rather than fully static-cached pages.
